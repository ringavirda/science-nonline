...
* ``fit_eda_adaptive`` -- curvature-adaptive EDA windows (#6), validated on
...
# --- #6 adaptive-window EDA ------------------------------------------------- #
def test_adaptive_eda_recovers_transient():
    rng = np.random.default_rng(4)
    t = np.linspace(0, 3, 400)
    y = 2.0 * (1 - np.exp(-3.0 * t)) + rng.normal(0, 0.02, t.size)
    r = fit_eda_adaptive(t, y, "K*(1-exp(-a*x))", "x", p0=[1.0, 1.0])
    assert abs(r.coeffs[0] - 2.0) < 0.2 and abs(r.coeffs[1] - 3.0) < 0.5


def test_adaptive_eda_equal_mode_runs():
    rng = np.random.default_rng(5)
    t = np.linspace(0, 3, 300)
    y = 1.5 * (1 - np.exp(-2.0 * t)) + rng.normal(0, 0.02, t.size)
    r = fit_eda_adaptive(t, y, "K*(1-exp(-a*x))", "x", window_mode="equal",
                         p0=[1.0, 1.0])

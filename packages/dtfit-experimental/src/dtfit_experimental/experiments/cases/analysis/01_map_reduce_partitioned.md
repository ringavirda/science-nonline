...
with the channel-batching GEMM ([10_gemm_batched_projection.md](10_gemm_batched_projection.md))
gives `PartitionedBatchLSI` — *flat memory over volume **and** one matmul over
channels in one pass*, exact to machine precision, ~19× faster than looping this
...
at a flat 544 MB**, with throughput constant (~11 Msample/s) across an 8× volume
range:
...
**Parallel reduce (Exp 7):** threaded map-reduce reaches **2.06× at P=16** —
...
## Why the parallel speedup is "only" ~2×

The 2.06× at P=16 (Exp 7) is **not** an algorithmic limit — it is the roofline.
...
([11_gpu_backend.md](11_gpu_backend.md)): the bottleneck is data movement, never
the additions.
...
- **Not** a substitute for an online *tracker* — for time-varying parameters with
  drift, use the streaming filters ([09_streaming_filters.md](09_streaming_filters.md)).

## Related

- The projection it accumulates is now a GEMM — see
  [10_gemm_batched_projection.md](10_gemm_batched_projection.md).
- Pairs with the GIL-released kernels for the threaded reduce
  ([07_gil_released_kernels.md](07_gil_released_kernels.md)).

...
Where ``experiments/`` evaluates each EDA/LSI adaptation *in isolation* (one
...
* ``parameter_estimation`` -- physical-parameter recovery, merging batch LSI/EDA,
  adaptive-window EDA (transients) and joint multi-channel fitting behind a
  scenario-aware selector, against the NLLS gold standard.
* ``big_data`` -- batch *and* streaming, merging the promoted map-reduce
  estimators (PartitionedLSI/EDA), the fused multi-channel ``PartitionedBatchLSI``

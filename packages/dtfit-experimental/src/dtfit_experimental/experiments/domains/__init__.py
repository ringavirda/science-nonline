...
* the **hardware twin** of ``realtime_gps`` -- driving the real Arduino Nano 33
  BLE + NEO-M8N rig on real silicon -- now lives in its own ``dtfit-hardware``
  package (firmware, USB/BLE host link, phone app); see
  ``packages/dtfit-hardware``. It depends on this package's ``realtime_gps``.

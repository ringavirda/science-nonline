...
| Real-time GPS/inertial trajectory — **hardware rig** (real silicon) | [realtime_gps_hw.ipynb](realtime_gps_hw/realtime_gps_hw.ipynb) |
...
| Real-time GPS/inertial trajectory (hardware) | The **on-silicon** reproduction of the simulated rig: the streaming integral trackers running on an Arduino Nano 33 BLE Sense M4F over a live NEO-M8N fix stream + onboard IMU, with INA226 energy and BLE logging — validated against held-out forecasts, surveyed paths and **public RTK/INS datasets** (not a live-cm claim). Host control + telemetry in `realtime_gps_hw/backend.py`, firmware in `realtime_gps_hw/firmware/`; build in `papers/embedded_hardware_bom.md`. Turns the de-risked simulation into the embedded paper |

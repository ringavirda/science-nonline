...
*Generated by `embedded_control/run.py` on 2026-06-19.*
...
- **FilterBank + fused χ² detector** — a bank of per-axis filters whose one-step innovations pool into a χ²(n_axes) fault statistic, acted on via the `inflate` covariance re-arm; each filter also runs a NIS + CUSUM drift test.
...
| estimator | RMSE vs clean | param err % | physical params? | latency (µs) |
|---|---|---|---|---|
...
| first_order | LSIFilter | A saturating exponential is where the spectrum clearly helps: its several orthogonal coefficients pin (K, τ) far better than a single area, which leaves τ weakly constrained (3.8% vs ~19% param error). |
...
A 3-axis oscillator with a damping fault (ζ jumps on every axis at the midpoint). The bank of `LSIFilter`s pools its three one-step innovations into a fused χ²(3) statistic; on a detection it re-arms via `inflate`. We measure tracking error, detection latency, false alarms, and the marginal value of the `inflate` re-arm.
...
| estimator | state words | float32 B | window buffer? | params? | latency µs (n=3,W=60) |
|---|---|---|---|---|---|
...
| ARM Cortex-M0+ (SAMD21) | 32 KB | no (soft) | ✓ |
| ARM Cortex-M4F (STM32F4) | 192 KB | yes | ✓ |
| ESP32 (LX6 FPU) | 520 KB | yes | ✓ |
...
![Left: state is small and flat in stream length (grows only with window). Right: per-sample latency — all far under any real-time budget.](figures/footprint.png)
...
![Online one-step tracking of the hryvnia crisis: the filter follows the depreciation but, honestly, does not beat the random walk one-step on this near-RW series.](figures/realdata.png)
...
- **Fault detection + on-device adaptation.** The fused χ² detector flags a multi-axis fault within a window at low false-alarm rate (pooling axes raises the SNR), and the `inflate` re-arm measurably speeds recovery — online adaptation a fixed-gain filter or an offline-trained net cannot do.

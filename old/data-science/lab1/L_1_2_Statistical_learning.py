...
2. Парсинг реальних даних https://www.oschadbank.ua/rates-archive, Oschadbank (USD).xls;
3. Визначення статистичних характеристик навчальної вибірки;
4. Детекція та очищення навчальної вибірки від аномалій;
5. Статистичне навчання параметрів поліноміальної моделі за МНК - бібліотека numpy;
...
def randoNORM(dm, dsig, iter):
    ...
    dS = np.var(S)
    scvS = mt.sqrt(dS)
    print("------- статистична характеристики НОРМАЛЬНОЇ похибки вимірів -----")
    print("математичне сподівання ВВ=", mS)
    print("дисперсія ВВ =", dS)
    print("СКВ ВВ=", scvS)
    print("------------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    if Data_mode == 1:
        ...
        SV_AV = Model_NORM_AV(
            S0, SV, nAV, Q_AV
        )  # модель тренда + нормальних помилок + АВ
    ...
    if Data_mode == 2:
        ...
        SV_AV = file_parsing(
            "https://www.oschadbank.ua/rates-archive", "data\\Oschadbank (USD).xls", "Продаж"
        )  # реальні дані
    ...
    if mode == 2:
        ...
        # --------------- Очищення від аномальних похибок sliding window -------------------
        n_Wind = 5  # розмір ковзного вікна для виявлення АВ
        koef_Extrapol = 0.5  # коефіцієнт прогнозування: співвідношення інтервалу спостереження до  інтервалу прогнозування
        koef = mt.ceil(
            n * koef_Extrapol
        )  # інтервал прогнозу по кількості вимірів статистичної вибірки
        S_AV_Detect_sliding_wind = Sliding_Window_AV_Detect_sliding_wind(SV_AV, n_Wind)
        Stat_characteristics_in(
            S_AV_Detect_sliding_wind, "Вибірка очищена від АВ алгоритм sliding_wind"
        )

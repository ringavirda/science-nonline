...
def GET_test(url: str) -> None:
    ...
    )  # перевірка успішності доступу до серверу, успішна відповідь для get 200
...
def POST_test(url: str) -> None:
    ...
    data = {
        "title": "тестування_POST_для_консультації_5",
        "ОК": "_тестування_POST_успішне",
        "userId": 5,
    }
    ...
    )  # ініціалізація надання інформаціі до серверу
    print(
        response.status_code
    )  # перевірка успішності доступу до серверу, успішна відповідь для post 201
    print(response.text)  # відображення вмісту сторінки за url
...
def PUT_test(url: str) -> None:
    ...
    data = {
        "id": 1,
        "userId": 2,
        "title": "тестування_для_PUT_консультації_5",
        "body": "тестування_PU_успішне",
    }
    headers = {"Content-Type": "application/json; charset=UTF-8"}
    ...
    print(
        response.status_code
    )  # перевірка успішності доступу до серверу, успішна відповідь для post 200
    print(response.text)  # відображення вмісту сторінки за url
...
def DELETE_test(url: str) -> None:
    ...
    headers = {"Content-Type": "application/json; charset=UTF-8"}
    # ініціалізація доступу до серверу
...
def Parser_URL_rdk(url):
    ...
    quotes = soup.find_all(
        "div", class_="newsline"
    )  # вилучення із html документу ленти новин
...
if __name__ == "__main__":
    ...
    if mode == 4:
        print(
            "Обрано: Парсинг табличних даних https://www.worldometers.info/coronavirus/"
        )

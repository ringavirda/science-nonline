...
The EAC calls pin `active_ratio=0.8` — the study-tuned recipe this pipeline was
...
| `.to_series()` | method (v0.4) | the pandas "in → out" view: a `Series` of the forecast values indexed by `.index` (raises if `.index` is `None`) |

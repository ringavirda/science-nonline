...
Mauna Loa CO_2 (smooth rising trend + annual cycle), 20% holdout (Exp 4):
...
-> **Best method on CO_2**, and the only one with a positive R^2 -- it beat the LSTM
...
1. **CO_2 is genuinely additive: `trend(t) + season(t) + noise`.** The composite
...
3. **Extrapolation stays disciplined.** Both components are simple and bounded
   ...
   here actually went *negative* R^2 -- they overfit the short training span and
...
single fortunate fit. Boosting has one (CO_2). The LTSF benchmark was *not* a fair
second domain (those channels have no clean parametric trend+season -- see
[02_pluggable_basis.md](Cases-Analysis-02-Pluggable-Basis)). A fair second test would be
...
- Contrast with [#2 pluggable basis](Cases-Analysis-02-Pluggable-Basis): same Fourier

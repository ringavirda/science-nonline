...
Source: [`scale/_parallel.py`](https://github.com/ringavirda/science-nonline/blob/main/packages/dtfit/src/dtfit/scale/_parallel.py).
...
used as orchestration in [Noise (3)](https://github.com/ringavirda/science-nonline/blob/main/packages/dtfit-experimental/src/dtfit_experimental/experiments/cases/03_noise_robustness/03_noise_robustness.ipynb).
...
-> **Slower than serial** for these fine-grained fits (<=0.45x).
...
- **The threading backend** for kernel-bound work: threads have no spawn/pickle
  cost and the GIL-released kernels ([07_gil_released_kernels.md](Cases-Analysis-07-GIL-Released-Kernels))
...
**threaded GIL-released kernels (9.3x) > threaded map-reduce (2.1x, memory-bound)
>> fine-grained process fan-out (<=1x).** The lesson is to keep parallelism
**in-process** and **coarse**: parallelize the *kernels* and the *reduce*, not
thousands of tiny independent `lambdify`-bearing fits.

## Related

- The path that *does* scale: [07_gil_released_kernels.md](Cases-Analysis-07-GIL-Released-Kernels).

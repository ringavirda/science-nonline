...
Median EDA parameter-recovery error (%) over a noisexsize grid for the exponential family, the whole grid fitted in parallel across cores with `fit_many`.
...
- Under outliers **LSI is the standout**: its Savitzky-Golay pre-filter plus integral projection reject gross outliers, so it holds R^2~=0.9 even at 20% contamination -- matching or beating `curve_fit`, while stock EDA collapses.

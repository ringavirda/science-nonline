...
Throughput is ~constant and peak memory is flat across a 4x volume range -- the signature of an O(1)/sample estimator. Extrapolating the linear time fit (~6.82 s/GB at single-thread): **10 GB ~= 68 s, 40 GB ~= 273 s, 1 TB ~= 114 min** -- all at the same flat memory.

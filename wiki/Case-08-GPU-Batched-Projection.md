...
Measure the recommendation-#2 reframe: expressing the LSI/EDA projection integraly*phi_j as a single GEMM `S = Dᵀ.(w⊙Y)` over many channels. We quantify (1) the batched-GEMM speedup over a per-channel loop on CPU, (2) the fp32 vs fp64 throughput (the kernel is bandwidth-bound), and (3) the honest GPU story -- resident vs streamed -- for every available backend. The projection is exact (identical to the per-channel loop), so any speedup is real, not a corner cut.
...
| channels B | loop (ms) | batched (ms) | speedup x | Melem/s | GB/s | max|Delta| |
|---|---|---|---|---|---|---|
...
| backend | dtype | resident (ms) | streamed (ms) | streamed/resident x | Melem/s (resident) |
|---|---|---|---|---|---|
...
- **Use fp32 -- consumer fp64 is throttled.** GeForce cards run double precision at ~1/64 of single, so *resident fp64* is only ~2x the CPU (7,772 vs 4,426 Melem/s). *Resident fp32* (full-rate) jumps to 11,429 Melem/s -- **3x the fp64 CPU**, 2x the fp32 CPU, 1x the GPU's own fp64.
- **fp32 resident saturates GPU memory bandwidth.** That 11,429 Melem/s is ~47 GB/s of reads -- near the card's ~960 GB/s GDDR7 peak. So the win is not magic: it is exactly the **bandwidth ratio** (~1x the CPU's 52 GB/s), the textbook outcome for a bandwidth-bound reduction once the data is on the device.
The streamed story still holds: transferring `Y` per call is **4x slower** than resident (~11 GB/s, PCIe-bound, ~= the CPU's 52 GB/s), because every byte crosses PCIe for ~7 FLOPs of work. The GPU is decisive for **device-resident / many-channel** projection and offers ~nothing for a single streaming pass over out-of-core data, where partition-and-reduce (O(order) state) stays the lever.
...
- **Batching is the CPU win.** Projecting all channels in one GEMM (building the basis once, then BLAS) is up to ~21x faster than looping the per-channel call, and the result is equal to the loop to machine precision -- no GPU required.

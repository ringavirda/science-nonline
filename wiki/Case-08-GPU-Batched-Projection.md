...
Measure the recommendation-#2 reframe: expressing the LSI/EAC projection integraly*phi_j as a single GEMM `S = Dᵀ.(w⊙Y)` over many channels. We quantify (1) the batched-GEMM speedup over a per-channel loop on CPU, (2) the fp32 vs fp64 throughput (the kernel is bandwidth-bound), and (3) the honest GPU story -- resident vs streamed -- for every available backend. The projection is exact (identical to the per-channel loop), so any speedup is real, not a corner cut.

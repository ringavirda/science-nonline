...
With gross **evenly-scattered** outliers, plain EAC's integral averaging is already far more robust than a pointwise LSI/NLLS, but the dedicated **robust NLLS (soft-L1) is the clear winner** on this sweep -- the honest verdict that scattered outliers want a robust loss, not window ensembling. The #3 overlapping-window ensemble (`ensemble_fit`, **promoted to stable `dtfit`**) is the *complementary* tool for a different corruption mode: it takes the median over per-window fits and so rejects whole outlier-corrupted **windows** (a burst that wipes out a contiguous stretch), rather than down-weighting sprinkled point spikes. It is a specialized whole-window-rejection instrument, the complement to `fit_eac`'s per-residual robust loss -- not a general default, and by design it does not separate from that robust loss on evenly-scattered outliers like these.

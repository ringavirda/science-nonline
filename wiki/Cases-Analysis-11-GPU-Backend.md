...
GPU is decisive when data is **device-resident** (fp32: ~16x the CPU,
...
Source: [`_core/_backend.py`](https://github.com/ringavirda/science-nonline/blob/main/packages/dtfit/src/dtfit/_core/_backend.py),
...
([10_gemm_batched_projection.md](Cases-Analysis-10-GEMM-Batched-Projection)) runs unchanged on
...
|---|---|---|---|---|---|
| numpy | fp64 | 57 | 57 | 1.0x | 7,165 |
| numpy | fp32 | 31 | 36 | 1.2x | 13,071 |
| cupy | fp64 | 32 | 211 | 6.7x | 12,967 |
| **cupy** | **fp32** | **1.9** | 94 | **49x** | **212,956** |

- **fp32 resident: ~16x the fp32 CPU, ~30x the fp64 CPU.**
- fp32 resident achieves **~853 GB/s ~= 88% of the card's GDDR7 peak**.
- fp64 resident: only **~2x** the CPU.
...
1. **Resident fp32 wins by exactly the bandwidth ratio.** 853 GB/s on the GPU vs
   ~54 GB/s on the CPU is ~16x, and the measured speedup is ~16x. This is the
...
2. **Streamed collapses to PCIe.** If `Y` must be copied host->device every call,
   ...
   which is *comparable to or below* CPU memory bandwidth. Hence streamed is 49x
...
3. **Consumer fp64 is hardware-throttled (~1/64).** GeForce cards gate double
   precision, so fp64 resident is only ~2x the CPU (compute-limited by the
...
reduce estimator ([01_map_reduce_partitioned.md](Cases-Analysis-01-Map-Reduce-Partitioned)),
...
`PartitionedBatchLSI` ([12_fused_streaming_batched.md](Cases-Analysis-12-Fused-Streaming-Batched))
is backend-pluggable, so its per-chunk GEMM was run on the GPU directly -- and it
**did not help** (measured 0.6-0.9x the CPU). This is the same roofline result
...
- Rides on the CPU-side GEMM reframe: [10_gemm_batched_projection.md](Cases-Analysis-10-GEMM-Batched-Projection).
- The complementary streamed big-data path: [01_map_reduce_partitioned.md](Cases-Analysis-01-Map-Reduce-Partitioned).

...
- **`benchmark.py`** renders the per-method figures into the repo's `wiki/figures/`
  and prints the comparison tables shown on the [Methods](Methods) wiki pages
  (needs the optional `bench` extra: matplotlib).
...
- LSI/EAC need a **modest dynamic range** (their empirical spectrum is a Maclaurin
  ...
  before fitting (invertible rescalings that do not affect R^2).

...
`docs/methods/figures/` and prints the comparison tables shown on the
...
A 4-week take-off window (~16x range, where a pure exponential is
well-conditioned). LSI and EDA recover the growth on par with a SciPy
`curve_fit` baseline (in-sample **R^2 ~= 0.97 / 0.98**, MAPE ~= 5%). Forecasting
the held-out last 20% gives **negative R^2** for *all three* methods (including
...
R^2 ~= 0.78), confirming the sklearn integration.
...
**MAPE ~= 12%** (R^2 ~= 0.6); the residual is the two discrete devaluation jumps a
single smooth exponential cannot represent. Honest, interpretable.

**2b. EDAFilter streaming -- online tracking, bounded cost per sample.**
...
- Domains are normalized to `[0, 1.5]` and series scaled to O(1) before
  fitting; these are invertible rescalings and do not affect R^2.

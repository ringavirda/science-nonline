...
Model `y = A.e^(-ζω t).sin(ω√(1-ζ²).t)`, truth A=2.0, omega=3.0, zeta=0.15, n=240, 5% noise. Error columns are against the *clean* signal; **param err %** is the mean relative error of the recovered (A, omega, zeta) -- the quantity a control engineer cares about (the MLP fits the curve but recovers no physical parameters).
...
Model `y = K.(1 - e^(-t/τ))`, truth K=3.0, tau=1.2.
...
A 3-output plant shares one natural frequency omega. `fit_joint` estimates the shared omega from *all* outputs at once (plus each output's private amplitude/damping) in a single coupled system, halving the parameter count and guaranteeing a *consistent* omega across channels.

...
Each of P threads runs a fixed batch of native Simpson-kernel calls on cache-resident data (compute-bound). Because the kernels release the GIL, P threads do Px the work in nearly the same wall time -- a near-linear **throughput** multiplier. This is the direct validation of the GIL-release kernel change.
...
Near-linear to the physical-core count (peak **7.6x** at P=8), tapering past 16 cores (SMT). Amdahl serial fraction s~=**0.008**.
...
![Acceleration vs parallelism rank (three workloads).](figures/figures-scaling.png)

*Acceleration vs parallelism rank (three workloads).*

## Reading it

- **Compiled kernels scale near-linearly** (7.6x at P=8) -- the GIL-release refactor lets dtfit's hot numeric loops use every physical core, the key result for productivity on real hardware.
- The threaded **map-reduce** stream scales to ~2.5x before memory bandwidth saturates (expected for a streaming, data-bound workload).
- Fine-grained **`fit_many`** fits are embarrassingly parallel in principle but here limited (~1.0x) by per-task process overhead; they scale when the per-task work is coarse/heavy (batch the fits) rather than millisecond-sized. The ceilings are set by the platform (IPC, memory bandwidth), not the algorithms.

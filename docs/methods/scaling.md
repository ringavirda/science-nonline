...
> [`scale/_partitioned.py`](../../packages/dtfit/src/dtfit/scale/_partitioned.py),
> [`scale/_batched.py`](../../packages/dtfit/src/dtfit/scale/_batched.py),
> [`scale/_parallel.py`](../../packages/dtfit/src/dtfit/scale/_parallel.py).
> `PartitionedLSI`, `PartitionedEDA`, `PartitionedBatchLSI`, `fit_lsi_batched`,
> `project_spectra`, `fit_many`. API: [../api/scaling.md](../api/scaling.md).

The fitting *math* of [LSI](lsi.md)/[EDA](eda.md) is unchanged here; these are
...
[EDA](eda.md): a window's area is additive over the samples that fall in it.
...
pluggable [Backend](../api/scaling.md)); the projection has low arithmetic
...
returning one [`FittingResult`](../api/types.md) per channel. Maximal throughput,
...
[`FittingProblem`](../api/scaling.md#fittingproblem) is picklable and a failed fit
...
[`BatchFittingResult`](../api/scaling.md#batchfittingresult)s.
...
[LSI](lsi.md)/[EDA](eda.md). For real-time *online* tracking (as opposed to batch
at scale) use the [streaming filters](legendre_filter.md).

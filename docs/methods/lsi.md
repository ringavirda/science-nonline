...
> [`methods/_lsi.py`](../../packages/dtfit/src/dtfit/methods/_lsi.py); the shared
> orthogonal-basis machinery is in
> [`_core/_spectral.py`](../../packages/dtfit/src/dtfit/_core/_spectral.py).
...
- **frequency seeded** from the data's FFT peak via
  [`fft_frequency_seed`](../api/fitting.md#fft_frequency_seed) — the local solve
...
see [../experimental/adaptations-api.md](../experimental/adaptations-api.md)), and
the **same** machinery powers the scale backends ([scaling.md](scaling.md)) — where
...
[LSIFilter](legendre_filter.md) / [EDAFilter](equal_areas_filter.md); for the most
noise-robust batch fit with few parameters, [EDA](eda.md); at scale (one-pass,
distributed, or many-channel), the [partitioned / batched backends](scaling.md).

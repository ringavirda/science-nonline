...
> [`auto.py`](../../packages/dtfit/src/dtfit/auto.py).
> `auto_estimate(x, y, expr, var, ...)`, `auto_forecast(x, y, horizon, ...)`.
> API: [../api/auto.md](../api/auto.md).
...
|---|---|---|
| **oscillatory** (`freq_param` given, or FFT power share > 0.10) | [`fit_lsi`](lsi.md) oscillatory recipe | the spectrum (not an area) observes a cycle |
| **transient / peak** | [`fit_eda_adaptive`](eda.md) (curvature windows) | windows concentrate on the bend |
| **robust** (outliers) | [`fit_eda`](eda.md) with `loss="soft_l1"` | area integration + window down-weighting |
...
[`FittingResult`](../api/types.md) from the selected estimator.
...
use [`fit_lsi`](lsi.md) with **data-driven seeds and bounds** (the growth rate is
bracketed by the time span; the seasonal frequency is seeded from
[`fft_frequency_seed`](../api/fitting.md#fft_frequency_seed)), so the global search
is well-posed.
...
into a usable default. They compose only stable pieces ([`fit_lsi`](lsi.md),
[`fit_eda`](eda.md), [`fit_eda_adaptive`](eda.md),
[`fft_frequency_seed`](../api/fitting.md#fft_frequency_seed)) — the conservative
...
call the individual methods ([LSI](lsi.md), [EDA](eda.md)); for *model* inference
(which family fits at all) use [`suggest_models`](../api/models.md). The honest

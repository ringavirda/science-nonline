# EqualAreasFilter — recursive (streaming) EDA

> Numeric **online** method — the deployable real-time path. Source:
> [`streaming/equal_areas.py`](../../src/dtfit/streaming/equal_areas.py).
> Invoke via `dt.EqualAreasFilter(expr, var, p0=, window_size=, ...)` then
> `flt.partial_fit(t, y)` per sample; `flt.predict(x)`, `flt.params_`.

The EqualAreasFilter runs [EDA](eda.md)'s equal-areas identification
...
| method | R² | RMSE | MAPE % |
|---|---|---|---|
| EqualAreasFilter (lag-1 tracking) | 0.7933 | 2.483 | 9.22 |
| EqualAreasFilter (1-step-ahead) | 0.7883 | 2.51 | 9.36 |
...
**Use the EqualAreasFilter for:** real-time / streaming control loops

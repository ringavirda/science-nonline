...
[`FittingResult`](types.md) (or anything with `.predict` / `.coeffs`) and report
...
- [`fit_report`](#fit_report) — goodness-of-fit + parsimony (R²/RMSE/AIC/BIC)
...
| `rss`, `rmse`, `r2` | residual sum of squares, root-mean-square error, R² |
...
keep the lowest IC (this is exactly what [`suggest_models`](models.md#suggest_models)
does internally).
...
- `Display.from_estimator(estimator, X, y, *, ax=None, **kwargs)` — from a fitted
  estimator (e.g. a [`NonlineRegressor`](estimator.md)) and data

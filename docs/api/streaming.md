...
[`fit_eda`](fitting.md#fit_eda), `LSIFilter` ↔ [`fit_lsi`](fitting.md#fit_lsi).
...
| `cusum_k` | `0.5` | CUSUM slack in innovation σ — smallest sustained shift to ignore; `inf` disables CUSUM |
...
- `update(t, y) -> bool` — ingest one sample per stream; returns `True` iff this
  step raises a fault. `statistic_` (current χ²), `flag_`, `flags_` (flagged step

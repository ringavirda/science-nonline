...
```bash
python experiments/download_data.py      # fetch the datasets into experiments/data/
python experiments/validate_methods.py   # fit + report metrics
python experiments/benchmark.py          # method docs: figures + comparison tables
```

`benchmark.py` additionally writes the per-method figures into
`docs/methods/figures/` and prints the comparison tables embedded in the
[method docs](../docs/methods/README.md). It needs the optional `viz` extra
(matplotlib) on top of the core deps.
...
well-conditioned). LSI and EDA recover the growth on par with a SciPy
...
**2a. USD/UAH exponential depreciation trend — batch LSI/EDA.** A single
...
**2b. EqualAreasFilter streaming — online tracking, bounded cost per sample.**
...
- LSI/EDA need a **modest dynamic range** (their empirical spectrum is a

...
(see :mod:`dtfit.extra.dt.taylor`). For a polynomial those coefficients are just
...
def fit_dsb(
    ...
    *,
    ...
    coeffs = _solve_balance(informative, params, n, options, p0)
    echo_if(options, "DSB fitted coefficients:", coeffs)
    ...
    subs_map = dict(zip(params, coeffs))
...
def _solve_balance(
    ...
    try:
        solutions = solve_nonline(square, params)
        ...
        echo_if(
            options,
            f"Symbolic balance solve failed ({exc}); using numeric least "
            "squares.",
        )
        return solve_numeric(balance, params, guess)

    if len(balance) > n:
        echo_if(options, "Overdetermined balance; refining numerically.")
        return solve_numeric(balance, params, best)
    return best

...
class EqualAreasFilter:
    ...
    def __init__(
        ...
        *,
        ...
        self.params = sorted(
            (s for s in model.free_symbols if s != t_sym), key=lambda s: s.name
        )
        n = len(self.params)
        if n == 0:
        ...
        self._f = sp.lambdify([t_sym, *self.params], model, "numpy")
        self._jac = [
    ...
    def _area_jac(self, j: int, t_arr: np.ndarray) -> np.ndarray:
        ...
        if np.isscalar(d):
            d = np.full_like(t_arr, float(d))
        return d

    def partial_fit(self, t_new: float, y_new: float) -> "EqualAreasFilter":
        ...
        # Jacobian row is the vector of integrated parameter sensitivities.
        np_params = len(self.p)
        m_func = self._f(t_arr, *self.p)
        sub = self._subwindows(t_arr.size)
        e_vec = np.empty(len(sub))
        h_mat = np.zeros((len(sub), np_params))
        for k, (a, b) in enumerate(sub):
            ts, ys, ms = t_arr[a:b], y_arr[a:b], m_func[a:b]
            e_vec[k] = simpson(y=ys, x=ts) - simpson(y=ms, x=ts)
            for j in range(np_params):
                h_mat[k, j] = simpson(y=self._area_jac(j, ts), x=ts)
        ...
        self.p = self.p + gain @ e_vec
        self.P = (np.eye(np_params) - gain @ h_mat) @ self.P + self.Q
        if self.adapt_r:
    ...
    def _drift_step(self, e: float) -> bool:
        ...
        if nis_drift or cusum_up or cusum_down:
            self._on_drift(up=cusum_up or (nis_drift and z > 0))
            return True
        return False
